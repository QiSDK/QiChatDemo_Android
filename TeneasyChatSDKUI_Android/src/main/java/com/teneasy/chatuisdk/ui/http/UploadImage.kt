...
data class ImageRequestBody (val filename: String, val assetKind: Int, val assetType: Int)

class UploadImage(lis: UploadListener) {
    private var listener: UploadListener? = null
    private val imageTypes =
        arrayOf("tif", "tiff", "bmp", "jpg", "jpeg", "png", "gif", "webp", "ico", "svg")
    ...
    fun uploadFile(file: File) {
        ...
        //Thread(Runnable {
         //   kotlin.run {
        var body = ImageRequestBody(file.name, 4, 1)
        val gson = Gson()
        var bodyStr = gson.toJson(body)
        val requestBody: RequestBody = bodyStr.toRequestBody("application/json".toMediaTypeOrNull())

        val request2 =
            Request.Builder().url(Constants.baseUrlApi() + "/v1/assets/upload-file")
    ...
    private fun subscribeToSSE(url: String, file: File) {
        ...
        val client = OkHttpClient.Builder()
            ...
            .readTimeout(
                0,
                TimeUnit.SECONDS
            )      // Set readTimeout to 0 for long-lived connections
            .build()
                        ...
                        for (line in lines) {
                            if (line.startsWith("event:", ignoreCase = true)) {
                                ...
                                if (result.percentage == 100 && result.data != null) {
                                    ...
                                    Log.i(
                                        TAG,
                                        (Date().toString() + "上传进度 " + result.percentage)
                                    )
                                } else {
