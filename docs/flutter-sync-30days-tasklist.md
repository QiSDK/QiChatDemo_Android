...
- [x] **D5 File_cell 在线预览**：核实结果——`WebViewActivity.onCreate` 内部本来就把收到的 URL 包进 `view.officeapps.live.com/op/view.aspx?src=`，Android 早已是 Office 在线预览，与 Flutter 一致，`onOpenFile` 传原始 URL 即可（评审发现并回滚了一次在 `onOpenFile` 再包一层造成双重包装的错误改动）。pdf→PdfViewer、csv→toast 保持不变。
